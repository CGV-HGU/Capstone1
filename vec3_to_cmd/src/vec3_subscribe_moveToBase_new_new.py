...
# 맵 y축 크기. hybrid A*랑 2D 맵과 origin이 달라서 이렇게 차이 보정 해줘야 한다
# x축은 보정 안 해줘도 된다
...
def roi_check_callback(msg):
    global floor_detected
    rospy.loginfo(f"Received ROI check: {msg.data}")
    if(msg.data == True):
        floor_detected = 0
    else:
        floor_detected = 1
    rospy.loginfo("check: {}".format(floor_detected))
...
if __name__ == '__main__':
    try:
        ...
        rospy.Subscriber('roi_check', Bool, roi_check_callback)
