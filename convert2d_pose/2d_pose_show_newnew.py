...
class RobotTracker:
    def __init__(self, map_image_path):
        ...
        self.currpos_rviz_pub = rospy.Publisher('current_rviz_pose', PoseStamped, queue_size=10)
    ...
    def publish_robot_pose_rviz(self):
        new_msg = PoseStamped()
        new_msg.header.stamp = rospy.Time.now()
        new_msg.header.frame_id = "world"
        
        new_msg.pose.position.x = self.robot_pose[0]
        new_msg.pose.position.y = self.map_val - self.robot_pose[1]
        ...
        new_msg.pose.orientation.w = quaternion[3] #Heading 
        
        self.currpos_rviz_pub.publish(new_msg)
                ...
                if self.state in [State.IDLE, State.TRACKING]:
                    if event.type == pygame.MOUSEBUTTONDOWN:
                        if event.button == 1:  # Left mouse button
                            ...
                            # self.twi1004_OH+NTH.pngst_pub.publish(twist_msg)
...
if __name__ == "__main__":
    map_image = "/home/cgvlab/catkin_ws/src/convert2d_pose/src/1007_OH+NTH_ver1.png"
